...
describe("Get Informations", () => {
  ...
  it('should be able to get Categories', async () => {
    const repositories = await getCategories();


    expect(repositories).toHaveProperty('data')
    expect(repositories).toHaveProperty('links')
  });
  ...
  it('should be able to get Cities in State', async () => {
    const cities = await getCitiesInState('SP')
